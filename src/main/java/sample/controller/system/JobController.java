 ...
 * low: 社内/バッチプロセス切り出し時はVM分散時の情報/排他同期を意識する必要があります。(DB同期/メッセージング同期/分散製品の利用 等)
 * low: メソッドは全てPOSTが望ましいですが、デモ用で叩きやすいようにGETを利用しています。
 *
 * @author jkazama
 */
@RestController
...
public class JobController {
	...
	/** 営業日を進めます。 */
	@RequestMapping(value = "/daily/processDay", method = RequestMethod.GET)
	public void processDay() {
		master.processDay();
	}

	/** 振込出金依頼を締めます。 */
	@RequestMapping(value = "/daily/closingCashOut", method = RequestMethod.GET)
	public void closingCashOut() {
		asset.closingCashOut();
	}

	/** キャッシュフローを実現します。 */
	@RequestMapping(value = "/daily/realizeCashflow", method = RequestMethod.GET)
